...
static void bfsMark(Graph* graph, const int* portalPartner, int* queue, int width, int height, int startIdx);
static void visitTile(Graph* graph, int* queue, int* tail, int idx);
...
static void buildGraph(const ParsingInfo* info, Graph* graph)
{
    ...
    bfsMark(graph, info->portalPartner, queue, info->width, info->height, info->horseIdx);
    ...
    if (info->unicornIdx != -1 && IS_NODE_NULL(graph->nodes[info->unicornIdx]))
        bfsMark(graph, info->portalPartner, queue, info->width, info->height, info->unicornIdx);
...
static void bfsMark(Graph* graph, const int* portalPartner, int* queue, int width, int height, int startIdx)
{
    ...
    // init start
    visitTile(graph, queue, &tail, startIdx);
